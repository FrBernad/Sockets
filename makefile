...
cleanTest:
	rm -rf output.cppOut report.tasks result.valgrind

test: clean 
	./pvs.sh; valgrind --leak-check=full -v ./server 2>> result.valgrind; valgrind --leak-check=full -v ./client 2>> result.valgrind; cppcheck --quiet --enable=all --force --inconclusive server.c levels.c client.c 2>> output.cppOut
